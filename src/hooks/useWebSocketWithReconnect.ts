...
export default function useWebSocketWithReconnect(options: WebSocketOptions) {
  ...
  const connect = () => {
    ...
    wsRef.current.onmessage = (event) => {
      if (!isActiveRef.current) return;
      // Defense-in-depth: confirm the message actually originated from the
      // origin we connected to, in case of TLS-terminating proxy misconfig.
      const expectedOrigin = new URL(url, window.location.href).origin;
      if (event.origin && event.origin !== expectedOrigin) {
        console.warn('[WS] Ignored message from unexpected origin:', event.origin);
        return;
      }
      try {
